...
async def get_for_user(db: AsyncSession, session_id: int, user_id: int) -> ChatSession | None:
    result = await db.execute(
        select(ChatSession)
...
async def append_message(db: AsyncSession, session_id: int, role: str, content: str) -> ChatMessage:
